package in.srain.cube.views.wt;
...
import in.srain.cube.views.wt.util.PtrCLog;
 ...
 * It defines {@link in.srain.cube.views.wt.PtrUIHandler}, which allows you customize the UI easily.
 */
public class PtrFrameLayout extends ViewGroup {
    ...
    private int mDurationToGoDown = 300;
    ...
    private boolean mDisableWhenWorkingAfterRelease = false;
    ...
    private int mMaxPullDownY = Integer.MAX_VALUE;
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.WTPtrFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.WTPtrFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.WTPtrFrameLayout_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.WTPtrFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.WTPtrFrameLayout_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.WTPtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);
            ...
            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.WTPtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.WTPtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    public boolean dispatchTouchEventSupper(MotionEvent e) {
        try {
            throw new IllegalArgumentException("block  -->" + e.toString());
        } catch (Exception ex) {
            if (DEBUG) {
                ex.printStackTrace();
            }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                //  <---------------------- 自定义添加 ---------------------->
                if (mDisableWhenWorkingAfterRelease && isWorkingAfterRelease()) {
                    return true;
                }
                //  <---------------------- 自定义添加 ---------------------->
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 减少一个触摸移动最小距离判断 避免与子View 的横向滑动相冲突
                if (mDisableWhenHorizontalMove
                        && !mPreventForHorizontal
                        && mPtrIndicator.isInStartPosition()
                        && Math.abs(offsetX) > Math.abs(offsetY) * mPtrIndicator.getResistance()) {
    ...
    private void onRelease(boolean stayForLoading) {
        ...
        if (mStatus != PTR_STATUS_LOADING
                && mPtrIndicator.getOffsetToGoDown() > mPtrIndicator.getOffsetToRefresh()
                && mPtrIndicator.getCurrentPosY() > mPtrIndicator.getOffsetToGoDown()) {
            if (mPtrIndicator.getCurrentPosY() >= mPtrIndicator.getPressedDownPos()) {
    ...
    private void onGoDown(boolean begin, boolean finish) {
        if (mPtrUIHandlerHolder.hasHandler()) {
            mPtrUIHandlerHolder.onUIRefreshGoDown(this, begin, finish);
            if (DEBUG) {
                PtrCLog.i(LOG_TAG, "PtrUIHandler: onUIRefreshGoDown");
            }
        }
    }
    ...
    /**
     * Scroll down to bottom if is not under touch
     */
    private void tryScrollBackToBottom() {
        if (!mPtrIndicator.isUnderTouch()) {
            mScrollChecker.tryToScrollTo(getMeasuredHeight(), mDurationToGoDown);
            onGoDown(true, false);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            ...
            onRelease(true);
        } else if (mPtrIndicator.getCurrentPosY() >= PtrFrameLayout.this.getMeasuredHeight()) {
            onGoDown(false, true);
        }
    }
    ...
    /**
     * Detect whether is working after release.
     *
     * @return
     */
    public boolean isWorkingAfterRelease() {
        return mStatus >= PTR_STATUS_LOADING;
    }

    public void refreshCompleteImmediately() {
        final int duration = getDurationToCloseHeader();
    ...
    public void autoDrop(int offset, int duration) {
        if (mStatus != PTR_STATUS_INIT) {
            return;
        }
        isAutoDrop = true;
        ...
        mScrollChecker.tryToScrollTo(offset, duration);
    }
     ...
     * @param disable
     */
    public void disableWhenWorkingAfterRelease(boolean disable) {
        mDisableWhenWorkingAfterRelease = disable;
    }
    ...
    public void setMaxPullDownY(int maxPullDownY) {
        mMaxPullDownY = maxPullDownY;
    }

    public int getMaxPullDownY() {
        return mMaxPullDownY;
    }

    public void setDurationToGoDown(int duration) {
        mDurationToGoDown = duration;
    }
    ...
    public void setOffsetToGoDown(int offset) {
        mPtrIndicator.setOffsetToGoDown(offset);
    }
